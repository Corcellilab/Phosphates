...
##########
def type_conver(atom_type):
    types = {1: 'P',
            2: 'O',
            3: 'OO',
            4: 'H',
            }
    return types[atom_type]

##########
def dimer_typing(lst, contacts,HbondParm,box_length):
    dimer_type = {'Classic':0,'Anti':0,'Parallel':0,'Other':0}
    for dimer in contacts:
        d1 = ['P','O','O','OO','OO','H','H']
        d2 = ['P','O','O','OO','OO','H','H']
        for i in lst:
            if i[5] == dimer[0]:
                atom_type = type_conver(int(i[1]))
                xyz = [float(i[2]),float(i[3]),float(i[4])]
                ind = d1.index(atom_type)
                d1[ind] = xyz
            elif i[5] == dimer[1]:
                atom_type = type_conver(int(i[1]))
                xyz = [float(i[2]),float(i[3]),float(i[4])]
                ind = d2.index(atom_type)
                d2[ind] = xyz
        
        d1 = np.array(d1,dtype=object)
        d2 = np.array(d2,dtype=object)
        dimer_distance = distance_matrix(d1,d2)
        
        p = {1:{1:0,2:0},2:{1:0,2:0}}
        #print(dimer_distance)
        for phos in range(1,3):
            for r in range(1,3):
                row = -r
                if phos == 1:
                    di = dimer_distance[row,:]
                elif phos == 2:
                    di = dimer_distance[:,row]         #row = column
                di = np.column_stack(np.where(di <= HbondParm[1]))
                #print(di)
                
                di_PBC = np.column_stack(np.where(di >= box_length-HbondParm[1]))
                #print(box_length-HbondParm[1])
                #print(di_PBC)
                for i in di_PBC:
                    og_cord = di[i[0]]
                    nxt_cord = di[i[1]]
                    new_cords = {0:og_cord[0], 1:og_cord[1], 2:og_cord[2]}
                    for _ in range(0,3):
                        og_dist = ((og_cord[_] - nxt_cord[_])**2)**(1/2)
                        dif1 = (((og_cord[_]+box_length) - nxt_cord[_])**2)**(1/2)      #cord + b
                        dif2 = (((og_cord[_]-box_length) - nxt_cord[_])**2)**(1/2)      #cord - b
                        if dif1 <= og_dist:
        ...
        total1 = sum(p[1].values())
        total2 = sum(p[2].values())
        total = (total1,total2)
        
        if total == (1,0) or total == (0,1):
            dimer_type['Classic'] += 1
        elif total == (1,1):
            dimer_type['Anti'] += 1
        elif total == (2,0) or total == (0,2):
            dimer_type['Parallel'] += 1
        else:
            dimer_type['Other'] += 1
...
def stepHbondCalc(lst, box_length, origin, bonds, donor_id, acceptor_id, Angles=[False,0], Gyra=False, branches=False, HbondParm=2.5):
    ...
    dimer_type = dimer_typing(lst, contacts, HbondParm, box_length)
    ...
    return [output,p,n[0],n[1],gyra_distances,branches,n[3],angles,distances,dimer_type]       #Return class data type rather than lists
...
def trjreader(filename, steps, bonds, donor_id, acceptor_id, XYZ=[False], binning=[False], Gyra=False, time_corr=[False,0], branches=False, ion_pairs=[False], Angles=[False,0], HbondParm=2.5):
    with open(filename) as f:
        ...
        while tot_steps < steps: #steps     #All calcs under 
            ...
            #StepHbondCalc returns [output,p,n[0],n[1],gyra_distances,branches,n[3],angles,distances,dimer_type]
            distances.append(calc[8])

            if Angles[0] == True:
                angle.append(calc[7])

            for k,v in  calc[9].items():
                dimer_type[k] += calc[9][k] 
