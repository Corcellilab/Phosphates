...
class CalcEjs(AnalysisBase):
    ...
    @staticmethod
    def add_edge(matrix, n1, n2):
        try: matrix[n1][n2]["weight"] += 1
        except KeyError: matrix[n2][n1] = {"weight": 1}
        ...
        for ej in ej_lst:
            ...
            if resid1 != resid2:
                if self.hbond == False:
                    ...
                    self.add_edge(filtered_ejs, resid1, resid2)
                    self.add_edge(filtered_ejs, resid2, resid1)

                    self.add_edge(atom_ejs, a1, a2)
                    self.add_edge(atom_ejs, a2, a1)

                    self.add_edge(atom_donor_adj, a1, a2)
                    
                    #mol_donor = directed weighted between molecules
                    self.add_edge(mol_donor_adj, resid1, resid2)
